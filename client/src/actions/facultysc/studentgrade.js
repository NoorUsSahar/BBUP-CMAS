...
export const getCurrentProfile = () => async (dispatch) => {
  try {
    const res = await axios.get("/studentgrade/me", {
      headers: {
...
export const getProfiles = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get("/api/studentgrade", {
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Headers": "X-Requested-With,content-type",
      },
...
export const getProfileById = id => async (dispatch) => {
  try {
    const res = await axios.get(`/api/studentgrade/student/${id}`);
    dispatch({
      type: GET_STUDENT_PROFILE,
      payload: res.data,
    });
...
export const addInfo = (formData, history, edit = false) => async (
  ...
  try {
    ...
    const res = await axios.post("/api/studentgrade", formData, config);
    ...
    if (errors) {
      ...
      dispatch(setAlert("Info not added , TRy Again"));
...
export const deleteAccount = (id) => async (dispatch) => {
  if (window.confirm("Are you sure")) {
    try {
      await axios.delete(`/api/studentgrade/`);
