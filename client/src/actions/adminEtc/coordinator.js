...
// Load Departments All Coordinators
export const loadDepartmentsAllCoordinator = id => async dispatch => {
  try {
    const res = await axios.get(`/api/coordinators/department/${id}`);

    dispatch({
      type: ALL_DEPARTMENT_COORDINATORS_LOADED,
      payload: res.data
    });
  } catch (err) {
    dispatch({
