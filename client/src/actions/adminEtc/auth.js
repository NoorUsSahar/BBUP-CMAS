...
export const loadUser = () => async dispatch => {
  ...
  try {
    const res = await axios.get('/api/adminAuth/me');
...
export const registerStudent = (formData, history) => async dispatch => {
  ...
  try {
    ...
    dispatch({
      type: REGISTER_UNDERGRADUATE_STUDENT_SUCCESS,
      payload: res.data.token
    });

    history.push('/coordinator/manage-applicant');

    dispatch(loadUser());
  } catch (err) {
    console.log(err.message);
    dispatch({ type: REGISTER_UNDERGRADUATE_STUDENT_FAILED });
...
export const registerFaculty =({ name, email, password} , history) => async (dispatch) =>{
  const config ={
   headers:{
       'Content-Type': 'application/json'
   }
...
try {
   const res = await axios.post('/api/adminFaculty' , body, config);

   dispatch({ 
       type : REGISTER_FACULTY_SUCCESS,
       payload : res.data //token
   });
   dispatch(setAlert("Faculty Registered" , 'danger'));
   history.goBack();
