...
import {
  addInfo,
  getCurrentProfile,
} from '../../../../actions/adminEtc/studentprofile';
...
const EditProfile = ({
  ...
  const {
    ...
    initialValues: {
      ...
      registrationNo:
        loading || !profile.registrationNo ? '' : profile.registrationNo,
      ...
      Olevels_matricMarks:
        loading || !profile.Olevels_matricMarks
          ? ''
          : profile.Olevels_matricMarks,
      alevels_fscMarks:
        loading || !profile.alevels_fscMarks ? '' : profile.alevels_fscMarks,
      feildOfStudy:
        loading || !profile.feildOfStudy ? '' : profile.feildOfStudy,
      yearOfMariculatioin:
        loading || !profile.yearOfMariculatioin
          ? ''
          : profile.yearOfMariculatioin,
      yearOfIntermediate:
        loading || !profile.yearOfIntermediate
          ? ''
          : profile.yearOfIntermediate,
      overseasEducation:
        loading || !profile.overseasEducation ? '' : profile.overseasEducation,
      section: loading || !profile.section ? '' : profile.section,
    },
    enableReinitialize: true,
    onSubmit: (values) => {
      addInfo(values, history, true);
    },
          ...
          <CardHeader color='primary'>
            <GridContainer>
              <GridItem xs={12} sm={12} md={6}>
                <h3 className={classes.cardTitleWhite}>Your Profile</h3>
