...
const Survey_Forms = require("../../../models/facultysc/Survey_Forms");
const Survey_Response = require("../../../models/facultysc/Survey_Response");
...
// @route  GET /api/survey/survey-end-date/:survey_form_id
// @desc   Get survey-end_date by survey id
// @access Private
router.get("/survey-end-date/:survey_form_id/", async (req, res) => {
  try {
    const survey_form = await Survey_Forms.findById(req.params.survey_form_id);

    if (!survey_form) return res.status(404).send("Survey not found");
const end_date= JSON.stringify(survey_form.end_date);
// const publish_date= JSON.stringify(survey_form.publish_date);
    res.json(survey_form.end_date);
  } catch (err) {
    console.error(err.message);
    if (err.kind == "ObjectId") {
      res.status(404).send("Survey not found");
...
// @route  GET /api/survey/survey-publish-date/:survey_form_id
// @desc   Get survey-publish-date by survey id
// @access Private
router.get("/survey-publish-date/:survey_form_id/", async (req, res) => {
  try {
    const survey_form = await Survey_Forms.findById(req.params.survey_form_id);

    if (!survey_form) return res.status(404).send("Survey not found");

    res.json(survey_form.publish_date);
  } catch (err) {
    console.error(err.message);
    if (err.kind == "ObjectId") {
      res.status(404).send("Survey not found");
