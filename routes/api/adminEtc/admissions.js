...
// @route  PUT /api/admissions/notification
// @desc   Add or update an notification to the session
// @access Private
router.put(
  '/notification',
  [
  ...
  async (req, res) => {
    try {
      ...
      session[0].notification = notification;
  ...
  try {
    const admission = await Admission.find().populate(
      ...
      [
        ...
        'feeDetails.challanPicture',
        'feeDetails.amountPaid',
        'applicantVerified',
        'applicantDiscarded'
      ]
    );
...
router.post(
  ...
  async (req, res) => {
    ...
    const { name, startDate, endDate, notification } = req.body;
    ...
    try {
      ...
      const sessionId = admission.sessions[admission.sessions.length - 1]._id;
...
// @route  DELETE /api/admissions/remove-admission-session/:id
