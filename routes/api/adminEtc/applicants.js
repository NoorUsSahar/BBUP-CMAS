...
const Applicant = require('../../../models/adminEtc/Applicant');
const auth = require('../../../middleware/adminEtc/auth');
const { check, validationResult } = require('express-validator');
const Programme = require('../../../models/adminEtc/Programme');
...
router.get('/', auth, async (req, res) => {
  try {
    const user = await Applicant.find({ isStudent: false })
      .select('-password')
...
router.get('/students', auth, async (req, res) => {
  try {
    const user = await Applicant.find({ isStudent: true })
      .select('-password')
      .populate('appliedPrograms', ['name', 'email']);
    res.json(user);
  } catch (err) {
...
// @route  PUT /api/applicants/register/:id
// @desc   Register a course
// @access Private
router.put('/register/:id/:app_id', auth, async (req, res) => {
  try {
    ...
    const applicant = await Applicant.findById(req.params.app_id);
    ...
    if (
      ...
      return res.status(400).json({ msg: 'Already applied for course' });
    }

    applicant.registeredCourses.push({ course: course.id });
...
// @route  PUT /api/applicants/remove/course/:id
// @desc   Remove a course
// @access Private
router.put('/remove/course/:id/:app_id', auth, async (req, res) => {
  try {
    const course = await Course.findById(req.params.id);
    ...
    const removeIndex = applicant.registeredCourses
      .map(course => course.course)
      .indexOf(req.params.id);

    if (removeIndex === -1) {
      return res.status(400).json({ msg: 'Course already removed' });
    }

    applicant.registeredCourses.splice(removeIndex, 1);
...
// @route  GET /api/applicants/check-criteria
...
// @desc   Forward the Applicants that match the criteria to the department
// @access Private
...
router.put(
  ...
  [
    auth,
    check('applicantVerified', 'Applicant verified is required').isBoolean()
  ],
  async (req, res) => {
    try {
      const applicant = await Applicant.findById(req.params.id);
...
// @route  PUT /api/applicants/pending/:id
// @desc   Change status to pending of an applicant
// @access Private
router.put(
  '/pending/:id',
  [
  ...
  async (req, res) => {
    try {
      ...
      applicant.applicantVerified = false;
...
// @route  PUT /api/applicants/discard/:id
// @desc   Change status of an applicant to discarded
// @access Private
router.put(
  '/discarded/:id',
  [
    auth,
    check('applicantDiscarded', 'Applicant discarded is required').isBoolean()
  ],
  async (req, res) => {
    try {
      const applicant = await Applicant.findById(req.params.id);

      applicant.applicantDiscarded = true;
...
// @route  PUT /api/applicants/acquire/:id
// @desc   Change status of an applicant to discarded
// @access Private
router.put(
  '/acquire/:id',
  ...
  async (req, res) => {
    try {
      ...
      applicant.applicantDiscarded = false;
...
// @route  PUT /api/applicants/ntsMarks
...
// @route  GET /api/applicants/ntsMarks/:id
...
// @route  PUT /api/applicants/submitted-fee-challan
// @desc   Upload submitted fee challan picture
// @access Private
router.put(
  '/submitted-fee-challan',
  [
    check('amountPaid', 'Amount is required')
      .not()
      .isEmpty(),
    check('challanPicture', 'Sbumitted challan picture is required')
      .not()
      .isEmpty(),
    auth
  ],
  async (req, res) => {
    ...
    const { amountPaid, challanPicture } = req.body;

    const feeDetails = {};

    feeDetails.amountPaid = amountPaid;
    feeDetails.challanPicture = challanPicture;
    ...
    try {
      ...
      applicant
        .populate('user', ['email'], (err, res) => {
          if (err) throw err;
          return res;
        })
        .populate(
          'appliedPrograms.programme',
          ['name', 'criteria'],
          (err, res) => {
            if (err) throw err;
            return res;
